...
# --- 1. CORE RULES - ANTI HALLUCINATION LOCK & VISUAL AID ENGINE ---
SYSTEM_PROMPT = """
...
# A. STATIC_FALLBACK_LIBRARY (The 11 core highly detailed SVGs)
...
# B. DYNAMIC CONFIGURATION & GENERATOR
CONFIG = {
    "density": {"draw": "beaker with water and object", "theory": "Density = Mass/Volume. Unit: kg/m³"},
    "conduction": {"draw": "metal rod in flame", "theory": "Heat transfer by vibrating particles in a solid."},
    "pinhole camera": {"draw": "box with hole, candle, inverted image", "theory": "Light travels in straight lines (Rectilinear propagation)."},
    "hooke's law": {"draw": "spring with weights", "theory": "Extension is proportional to applied force (F = kx)."},
    "ripple tank": {"draw": "tank with waves", "theory": "Demonstrates wave properties like reflection and refraction."},
    "resonance tube": {"draw": "tube in water", "theory": "Air column resonance used to measure speed of sound."},
    "electromagnet": {"draw": "coil with nail", "theory": "Current produces a magnetic field. Used in bells/relays."},
    "elasticity": {"draw": "spring force graph", "theory": "Materials return to original shape after stress is removed."},
    "electrostatics": {"draw": "2 charged rods", "theory": "Like charges repel, unlike charges attract."},
    "linear motion": {"draw": "car with v-t graph", "theory": "Gradient = acceleration. Area under graph = distance."}
}

def build_dynamic_svg(topic):
    """Generic SVG building blocks for new dynamic diagrams"""
    if topic == "density":
        return '''<svg viewBox="0 0 300 200" xmlns="http://www.w3.org/2000/svg">
            <rect x="100" y="50" width="100" height="120" fill="none" stroke="black" stroke-width="3" rx="5"/>
            <rect x="102" y="90" width="96" height="78" fill="lightblue" opacity="0.6"/>
            <rect x="130" y="120" width="40" height="40" fill="gray" stroke="black"/>
            <text x="140" y="145" fill="white" font-size="12">m</text>
            <text x="210" y="130" font-family="Arial" font-size="12">Volume (V)</text>
        </svg>'''
    elif topic == "conduction":
        return '''<svg viewBox="0 0 300 200" xmlns="http://www.w3.org/2000/svg">
            <rect x="50" y="90" width="200" height="20" fill="silver" stroke="black"/>
            <path d="M 50 110 Q 60 70 70 110 Z" fill="orange" stroke="red"/>
            <circle cx="100" cy="100" r="3" fill="red"/><circle cx="120" cy="100" r="3" fill="red"/>
            <circle cx="140" cy="100" r="3" fill="blue"/><circle cx="160" cy="100" r="3" fill="blue"/>
            <text x="180" y="80" font-family="Arial" font-size="12">Cold End</text>
            <text x="40" y="80" font-family="Arial" font-size="12">Heat</text>
        </svg>'''
    elif topic == "hooke's law":
        return '''<svg viewBox="0 0 300 200" xmlns="http://www.w3.org/2000/svg">
            <line x1="100" y1="20" x2="200" y2="20" stroke="black" stroke-width="4"/>
            <path d="M 150 20 L 150 40 L 140 50 L 160 70 L 140 90 L 160 110 L 150 120 L 150 140" fill="none" stroke="black" stroke-width="2"/>
            <rect x="135" y="140" width="30" height="30" fill="brown" stroke="black"/>
            <text x="142" y="160" fill="white" font-size="12">W</text>
            <line x1="190" y1="40" x2="190" y2="140" stroke="red" stroke-dasharray="4,2"/>
            <text x="200" y="95" fill="red" font-size="12">Extension (x)</text>
        </svg>'''
    elif topic == "electrostatics":
        return '''<svg viewBox="0 0 300 200" xmlns="http://www.w3.org/2000/svg">
            <rect x="50" y="80" width="80" height="20" rx="10" fill="lightblue" stroke="black"/>
            <text x="75" y="95" font-weight="bold" fill="black">++++</text>
            <rect x="170" y="80" width="80" height="20" rx="10" fill="lightpink" stroke="black"/>
            <text x="195" y="95" font-weight="bold" fill="black">----</text>
            <path d="M 140 90 L 160 90 L 155 85 M 160 90 L 155 95" fill="none" stroke="red" stroke-width="2"/>
            <text x="135" y="120" fill="red" font-size="12">Attraction</text>
        </svg>'''
    else:
        # Generic Placeholder for dynamically described topics
        return f'''<svg viewBox="0 0 300 200" xmlns="http://www.w3.org/2000/svg">
            ...
            <text x="150" y="100" text-anchor="middle" font-family="Arial" font-size="14" fill="#666">Generating diagram for:</text>
            <text x="150" y="125" text-anchor="middle" font-family="Arial" font-size="16" font-weight="bold" fill="#0056b3">{topic.upper()}</text>
        </svg>'''

def auto_generate_diagram(user_msg):
    """
    Analyzes the prompt, maps to STATIC or DYNAMIC config, and generates SVG + Theory.
    Special check for 'principle of moments' dynamic variables.
    """
    msg_lower = user_msg.lower()
    
    # 1. Check Principle of Moments (Requires dynamic variable extraction)
    if "moments" in msg_lower or "lever" in msg_lower:
        ...
        w1 = params.get('w1', 'W1'); w2 = params.get('w2', 'W2')
        d1 = params.get('d1', 'd1'); d2 = params.get('d2', 'd2')
        ...
        return svg, "Principle of Moments", {"theory": "For a body in equilibrium, sum of clockwise moments = sum of anticlockwise moments.", "extra_text": extra_text}

    # 2. Check Static Fallback Library
    for key, svg_data in STATIC_FALLBACK_LIBRARY.items():
        if key in msg_lower:
            return svg_data, key, {"theory": f"Diagram showing the concept of {key}.", "extra_text": ""}
            
    # 3. Check Dynamic Config
    for key, data in CONFIG.items():
        if key in msg_lower:
            return build_dynamic_svg(key), key, {"theory": data['theory'], "extra_text": f"Illustration: {data['draw']}"}
            
    # 4. Fallback if visual word is used but topic unknown
    match = re.search(r'(draw|show|diagram|illustrate|experiment)\s+([a-zA-Z0-9\s]+)', msg_lower)
    topic = match.group(2) if match else "Requested Topic"
    return build_dynamic_svg(topic), topic, {"theory": f"Conceptual diagram for {topic}", "extra_text": ""}
...
<div id="app-container">
    <div class="header">
        ...
        <p>Uganda UNEB Past Papers & NCDC 2026 Syllabus (S1-S4) | Auto-Illustration Engine</p>
    </div>
    
    <div class="main-content">
        <div id="chat-window">
            <div class="message ai-msg">Welcome! I can solve UNEB questions, explain topics, and <b>draw diagrams</b> on demand. Try asking: "draw a pinhole camera" or "UNEB 2019 Q3".</div>
        </div>
...
<script>
    ...
    async function sendMessage() {
        ...
        try {
            ...
            // Show explanation in theory box
            if (data.text) {
...
def chat():
    ...
    # Check for visual keywords trigger
    ...
    # Return both LLM response and dynamic visual aid
    return jsonify({
        "text": ai_response,
